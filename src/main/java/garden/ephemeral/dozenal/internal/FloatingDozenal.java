package garden.ephemeral.dozenal.internal;

import garden.ephemeral.dozenal.Dozenal;

import java.util.Arrays;
import java.util.regex.*;

/**
 * A class for converting between ASCII and dozenal representations of a single
...
public class FloatingDozenal {
    ...
    static final int INT_DOZENAL_DIGITS = 8;
    ...
    private static final BinaryToASCIIConverter B2AC_POSITIVE_ZERO = new BinaryToASCIIBuffer(false, new char[]{Dozenal.DIGIT_ZERO});
    private static final BinaryToASCIIConverter B2AC_NEGATIVE_ZERO = new BinaryToASCIIBuffer(true,  new char[]{Dozenal.DIGIT_ZERO});
    ...
    static class BinaryToASCIIBuffer implements BinaryToASCIIConverter {
        ...
        private void developLongDigits( int decExponent, long lvalue, int insignificantDigits ){
            if ( insignificantDigits != 0 ){
                ...
                long pow10 = FDBigInteger.LONG_6_POW[insignificantDigits] << insignificantDigits; // 10^i == 5^i * 2^i;
            ...
            if ( lvalue <= Integer.MAX_VALUE ){
                ...
                c = ivalue % Dozenal.RADIX;
                ivalue /= Dozenal.RADIX;
                while ( c == 0 ){
                    decExponent++;
                    c = ivalue % Dozenal.RADIX;
                    ivalue /= Dozenal.RADIX;
                ...
                while (ivalue != 0){
                    digits[digitno--] = Dozenal.getDigit(c);
                    decExponent++;
                    c = ivalue % Dozenal.RADIX;
                    ivalue /= Dozenal.RADIX;
                }
                ...
                c = (int)(lvalue % Dozenal.RADIX);
                lvalue /= Dozenal.RADIX;
                while (c == 0) {
                    decExponent++;
                    c = (int)(lvalue % Dozenal.RADIX);
                    lvalue /= Dozenal.RADIX;
        ...
        private void dtoa( int binExp, long fractBits, int nSignificantBits, boolean isCompatibleFormat)
        {
            ...
            //
            int dozExp = estimateDozExp(fractBits,binExp);
            int B2, B5; // powers of 2 and powers of 5, respectively, in B
            int S2, S5; // powers of 2 and powers of 5, respectively, in S
            int M2, M5; // powers of 2 and powers of 5, respectively, in M

            B5 = Math.max( 0, -dozExp );
            B2 = B5 + nTinyBits + binExp;

            S5 = Math.max( 0, dozExp );
            S2 = S5 + nTinyBits;
            ...
            if ( Bbits < 64 && tenSbits < 64){
                if ( Bbits < 32 && tenSbits < 32){
                    ...
                    int b = ((int)fractBits * FDBigInteger.SMALL_6_POW[B5] ) << B2;
                    int s = FDBigInteger.SMALL_6_POW[S5] << S2;
                    int m = FDBigInteger.SMALL_6_POW[M5] << M2;
                    int tens = s * Dozenal.RADIX;
                    //
                    ...
                    long s = FDBigInteger.LONG_6_POW[S5] << S2;
                    long m = FDBigInteger.LONG_6_POW[M5] << M2;
                    long tens = s * Dozenal.RADIX;
                    //
                    // Unroll the first iteration. If our decExp estimate
                    // was too high, our first quotient will be zero. In this
                    // case, we discard it and decrement decExp.
                ...
                //
                FDBigInteger Sval = FDBigInteger.valueOfPow62(S5, S2);
                int shiftBias = Sval.getNormalizationBias();
                Sval = Sval.leftShift(shiftBias); // normalize so that division works better

                FDBigInteger Bval = FDBigInteger.valueOfMulPow62(fractBits, B5, B2 + shiftBias);
                FDBigInteger Mval = FDBigInteger.valueOfPow62(M5 + 1, M2 + shiftBias + 1);

                FDBigInteger tenSval = FDBigInteger.valueOfPow62(S5 + 1, S2 + shiftBias + 1); //Sval.mult( 10 );
                //
                // Unroll the first iteration. If our decExp estimate
                // was too high, our first quotient will be zero. In this
                // case, we discard it and decrement decExp.
        ...
        private void roundup() {
            ...
            if (q == Dozenal.DIGIT_PENULTIMATE) {
                while (q == Dozenal.DIGIT_PENULTIMATE && i > firstDigitIndex) {
                    digits[i] = Dozenal.DIGIT_ZERO;
                    q = digits[--i];
                }
                if (q == Dozenal.DIGIT_PENULTIMATE) {
                    // carryout! High-order 1, rest 0s, larger exp.
                    decExponent += 1;
                    digits[firstDigitIndex] = Dozenal.DIGIT_ONE;
                    return;
                }
                // else fall through.
            }
            digits[i] = Dozenal.nextDigitAfter((char) q);
        ...
        private int getChars(char[] result) {
            ...
            if (decExponent > 0 && decExponent < 8) {
                ...
                if (charLength < decExponent) {
                    ...
                    Arrays.fill(result,i,i+charLength,Dozenal.DIGIT_ZERO);
                    i += charLength;
                    result[i++] = '.';
                    result[i++] = Dozenal.DIGIT_ZERO;
                } else {
                    result[i++] = '.';
                ...
                result[i++] = '.';
                if (decExponent != 0) {
                    Arrays.fill(result, i, i-decExponent, Dozenal.DIGIT_ZERO);
                    i -= decExponent;
                }
                ...
                if (e <= Dozenal.PENULTIMATE) {
                    result[i++] = Dozenal.getDigit(e);
                } else if (e <= Dozenal.RADIX_SQUARED_SUB1) {
                    result[i++] = Dozenal.getDigit(e / Dozenal.RADIX);
                    result[i++] = Dozenal.getDigit(e % Dozenal.RADIX);
                } else {
                    result[i++] = Dozenal.getDigit(e / Dozenal.RADIX_SQUARED);
                    e %= Dozenal.RADIX_SQUARED;
                    result[i++] = Dozenal.getDigit(e / Dozenal.RADIX);
                    result[i++] = Dozenal.getDigit(e % Dozenal.RADIX);
                }
            }
    ...
    static class ASCIIToBinaryBuffer implements ASCIIToBinaryConverter {
        ...
        @Override
        public double doubleValue() {
            int kDigits = Math.min(nDigits, MAX_DOZENAL_DIGITS + 1);
            //
            // convert the lead kDigits to a long integer.
            //
            // (special performance hack: start to do it using int)
            int iValue = Dozenal.getDigitValue(digits[0]);
            int iDigits = Math.min(kDigits, INT_DOZENAL_DIGITS);
            for (int i = 1; i < iDigits; i++) {
                iValue = iValue * Dozenal.RADIX + Dozenal.getDigitValue(digits[i]);
            }
            long lValue = iValue;
            ...
            while (true) {
                ...
                FDBigInteger bigB = FDBigInteger.valueOfMulPow62(bigBbits, B5, B2);
        ...
        public float floatValue() {
            ...
            } else if ((decExponent >= nDigits) && (nDigits + decExponent <= MAX_DOZENAL_DIGITS)) {
                //
         ...
         */
        private static final double[] SMALL_RADIX_POW = {
                1.0,
                1.0*12,
                1.0*12*12,
                1.0*12*12*12,
                ...
                1.0*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12,
                1.0*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12
        };
        ...
        private static final float[] SINGLE_SMALL_10_POW = {
                ...
                1.0f*12*12*12*12,
                1.0f*12*12*12*12*12,
                1.0f*12*12*12*12*12*12,
                1.0f*12*12*12*12*12*12*12,
                1.0f*12*12*12*12*12*12*12*12,
                1.0f*12*12*12*12*12*12*12*12*12,
                1.0f*12*12*12*12*12*12*12*12*12*12
        };

        private static final double DOZ_1E16 = 1.0*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12*12;
        private static final double[] BIG_10_POW = {
                DOZ_1E16,
                DOZ_1E16*DOZ_1E16,
                DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16,
                DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16,
                DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16*DOZ_1E16
        };
        private static final double DOZ_1EM16 = 1.0f/(12*12*12*12*12*12*12*12)/(12*12*12*12*12*12*12*12);
        private static final double[] TINY_10_POW = {
                DOZ_1EM16,
                DOZ_1EM16*DOZ_1EM16,
                DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16,
                DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16,
                DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16*DOZ_1EM16
        };

        private static final int MAX_SMALL_TEN = SMALL_RADIX_POW.length-1;
    ...
    static ASCIIToBinaryConverter readJavaFormatString( String in ) throws NumberFormatException {
        ...
        try{
            ...
            if(c == 'N') { // Check for NaN
                ...
                // something went wrong, throw exception
                break parseNumber;
            } else if (c == Dozenal.DIGIT_ZERO)  { // check for hexadecimal floating-point number
                if (len > i + 1 ) {
            ...
            if ( (i < len) &&  (((c = in.charAt(i) )=='e') || (c == 'E') ) ){
                int expSign = 1;
                int expVal  = 0;
                int reallyBig = Integer.MAX_VALUE / Dozenal.RADIX;
                ...
                while ( i < len  ){
                    ...
                    c = in.charAt(i++);
                    if(c>=Dozenal.DIGIT_ZERO && c<='9') {
                        expVal = expVal * Dozenal.RADIX + Dozenal.getDigitValue(c);
                    } else {
                        i--;           // back up.
...
//        return  s.replaceFirst("^0+", "");
        if(!s.isEmpty() && s.charAt(0)==Dozenal.DIGIT_ZERO) {
            for(int i=1; i<s.length(); i++) {
                if(s.charAt(i)!=Dozenal.DIGIT_ZERO) {
