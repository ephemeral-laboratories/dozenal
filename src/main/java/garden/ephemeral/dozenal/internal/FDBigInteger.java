...
public /*@ spec_bigint_math @*/ class FDBigInteger {
    ...
    @ }
    @
    @ public pure model static \bigint pow62(int p6, int p2) {
    @     ghost \bigint v = 1;
    @     for (int i = 0; i < p6; i++) v *= 6;
    @     return v << p2;
    @ }
    @
    @ public pure model static \bigint pow10(int p10) {
    @     return pow62(p10, p10);
    @ }
    @*/
    ...
    // Cache of big powers of 6 as FDBigIntegers.
    private static final FDBigInteger[] POW_6_CACHE;

    // Initialize FDBigInteger cache of powers of 6.
    static {
    ...
    public FDBigInteger(long lValue, char[] digits, int kDigits, int nDigits) {
        ...
        while (i < limit) {
            int ilim = i + 5;
            v = Dozenal.getDigitValue(digits[i++]);
            while (i < ilim) {
                v = Dozenal.RADIX * v + Dozenal.getDigitValue(digits[i++]);
            }
            multAddMe(Dozenal.RADIX_TO_5, v); // ... where 100000 is 10^5.
        }
        int factor = 1;
        v = 0;
        while (i < nDigits) {
            v = Dozenal.RADIX * v + Dozenal.getDigitValue(digits[i++]);
            factor *= Dozenal.RADIX;
        }
        if (factor != 1) {
            multAddMe(factor, v);
        }
     ...
     *
     * @param p6 The exponent of the power-of-five factor.
     * @param p2 The exponent of the power-of-two factor.
     * @return <code>6<sup>p6</sup> * 2<sup>p2</sup></code>
     */
    /*@
     @ requires p6 >= 0 && p2 >= 0;
     @ assignable \nothing;
     @ ensures \result.value() == \old(pow62(p6, p2));
     @*/
    public static FDBigInteger valueOfPow62(int p6, int p2) {
        if (p6 != 0) {
            if (p2 == 0) {
                return big6pow(p6);
     ...
     * @param p6 The exponent of the power-of-five factor.
     * @param p2 The exponent of the power-of-two factor.
     * @return <code>value * 6<sup>p6</sup> * 2<sup>p2</sup></code>
     */
    /*@
     @ requires p6 >= 0 && p2 >= 0;
     @ assignable \nothing;
     @ ensures \result.value() == \old(UNSIGNED(value) * pow62(p6, p2));
     @*/
    public static FDBigInteger valueOfMulPow62(long value, int p6, int p2) {
        assert p6 >= 0 : p6;
        assert p2 >= 0 : p2;
        int v0 = (int) value;
        int v1 = (int) (value >>> 32);
    ...
    /*@
     @ requires p2 >= 0;
     @ assignable \nothing;
     @ ensures \result.value() == pow62(0, p2);
     @*/
    ...
    public int quoRemIteration(FDBigInteger S) throws IllegalArgumentException {
        ...
        if (thSize < sSize) {
            ...
            int p = multAndCarryByRadix(this.data, this.nWords, this.data);
            if(p!=0) {
                this.data[nWords++] = p;
            } else {
                trimLeadingZeros();
    ...
    /*@
     ...
     @ ensures \result.value() == \old(this.value() * 10);
     @*/
    public FDBigInteger multByRadix() {
        if (nWords == 0) {
            return this;
        }
        if (isImmutable) {
            int[] res = new int[nWords + 1];
            res[nWords] = multAndCarryByRadix(data, nWords, res);
     ...
     * <code>6<sup>p6</sup> * 2<sup>p2</sup></code>. The operation will be
     ...
     */
    /*@
     @ requires this.value() == 0 || p6 == 0 && p2 == 0;
     @ assignable \nothing;
     @ ensures \result == this;
     @
     @  also
     @
     @ requires this.value() > 0 && (p6 > 0 && p2 >= 0 || p6 == 0 && p2 > 0 && this.isImmutable);
     @ assignable \nothing;
     @ ensures \result.value() == \old(this.value() * pow62(p6, p2));
     @
     @  also
     @
     @ requires this.value() > 0 && p6 == 0 && p2 > 0 && !this.isImmutable;
     @ assignable this.nWords, this.data, this.data[*];
     @ ensures \result == this;
     @ ensures \result.value() == \old(this.value() * pow62(p6, p2));
     @*/
    ...
    /*@
     @ requires p6 >= 0 && p2 >= 0;
     @ ensures \result == (this.value() < pow62(p6, p2) ? -1 : this.value() >  pow62(p6, p2) ? +1 : 0);
     @*/
    public /*@ pure @*/ int cmpPow62(int p6, int p2) {
        if (p6 == 0) {
            int wordcount = p2 >> 5;
            int bitcount = p2 & 0x1f;
    ...
    /*@
     ...
     @*/
    private static int multAndCarryByRadix(int[] src, int srcLen, int[] dst) {
        long carry = 0;
        for (int i = 0; i < srcLen; i++) {
            long product = (src[i] & LONG_MASK) * (long)Dozenal.RADIX + carry;
            dst[i] = (int) product;
            carry = product >>> 32;
        }
        return (int) carry;
    ...
    /**
     * Computes <code>6</code> raised to a given power.
     *
     * @param p The exponent of 6.
     * @return <code>6<sup>p</sup></code>.
     */
    private static FDBigInteger big6pow(int p) {
        assert p >= 0 : p; // negative power of 6
        if (p < MAX_SIX_POW) {
     ...
     * @return <code>6<sup>p</sup></code>.
     */
    private static FDBigInteger big6powRec(int p) {
        if (p < MAX_SIX_POW) {
            return POW_6_CACHE[p];
        }
        // construct the value.
        // recursively.
        int q, r;
        // in order to compute 6^p,
        // compute its square root, 6^(p/2) and square.
        // or, let q = p / 2, r = p -q, then
        // 6^p = 6^(q+r) = 6^q * 6^r
        q = p >> 1;
        r = p - q;
        FDBigInteger bigq = big6powRec(q);
        if (r < SMALL_6_POW.length) {
            return bigq.mult(SMALL_6_POW[r]);
        } else {
            return bigq.mult(big6powRec(r));
        }
    }
    ...
    public String toHexString(){
        if (nWords == 0) {
            return String.valueOf(Dozenal.DIGIT_ZERO);
        }
        ...
        for (int i = nWords -1; i >= 0; i--) {
            ...
            for(int j = subStr.length(); j < 8; j++) {
                sb.append(Dozenal.DIGIT_ZERO);
            }
            sb.append(subStr);
        }
        for(int i = offset; i > 0; i--) {
            sb.append(Dozenal.DIGIT_ZERO)
                    .append(Dozenal.DIGIT_ZERO)
                    .append(Dozenal.DIGIT_ZERO)
                    .append(Dozenal.DIGIT_ZERO)
                    .append(Dozenal.DIGIT_ZERO)
                    .append(Dozenal.DIGIT_ZERO)
