...
public class DozenalFormat extends NumberFormat {
    ...
    public DozenalFormat() {
        this.symbols = DozenalFormatSymbols.INSTANCE;
    }
    ...
    public final StringBuffer format(Object number,
        ...
        if (number instanceof Long ||
            ...
            return formatInternal((BigInteger)number, toAppendTo, pos);
    ...
    public StringBuffer format(double number, StringBuffer result, FieldPosition fieldPosition) {
        ...
        return formatInternal(number, result, fieldPosition);
    }
     ...
     * @param number    The double to format
     * @param result    where the text is to be appended
     * @param fieldPosition notified of locations of sub fields
     * @exception       ArithmeticException if rounding is needed with rounding
     *                  mode being set to RoundingMode.UNNECESSARY
     * @return The formatted number string
     */
    private StringBuffer formatInternal(double number, StringBuffer result, FieldPosition fieldPosition) {
    ...
    private StringBuffer formatInternal(long number, StringBuffer result, FieldPosition fieldPosition) {
        ...
        if (useBigInteger) {
            ...
            return formatInternal(bigIntegerValue, result, fieldPosition, true);
        }
     ...
     * @see java.text.FieldPosition
     */
    private StringBuffer formatInternal(BigDecimal number, StringBuffer result, FieldPosition fieldPosition) {
        fieldPosition.setBeginIndex(0);
        fieldPosition.setEndIndex(0);
        return formatInternal2(number, result, fieldPosition);
    }
     ...
     * @return The formatted number string
     */
    private StringBuffer formatInternal2(BigDecimal number, StringBuffer result, FieldPosition fieldPosition) {
        if (multiplier != 1) {
     ...
     * @see java.text.FieldPosition
     */
    private StringBuffer formatInternal(BigInteger number, StringBuffer result,
                                        FieldPosition fieldPosition) {
        fieldPosition.setBeginIndex(0);
        fieldPosition.setEndIndex(0);

        return formatInternal(number, result, fieldPosition, false);
    }
     ...
     */
    private StringBuffer formatInternal(BigInteger number, StringBuffer result,
                                        FieldPosition fieldPosition, boolean formatLong) {
        if (multiplier != 1) {
...
//            formatInternal(((Number)obj).longValue(), sb, delegate);
//        } else if (obj instanceof BigDecimal) {
//            formatInternal((BigDecimal)obj, sb, delegate);
//        } else if (obj instanceof BigInteger) {
//            formatInternal((BigInteger)obj, sb, delegate, false);
//        } else if (obj == null) {
...
//        return delegate.getIterator(sb.toString());
    ...
    private void resetFastPathData(boolean fastPathWasOn) {
        ...
        if (isFastPath) {
            ...
            fastPathData.fractionalMaxIntBound = isCurrencyFormat ? Dozenal.RADIX_SQUARED_SUB1 : Dozenal.RADIX_CUBED_SUB1;
            fastPathData.fractionalScaleFactor = isCurrencyFormat ? Dozenal.RADIX_SQUARED : Dozenal.RADIX_CUBED;
    ...
    private StringBuffer subformat(StringBuffer result, FieldPosition fieldPosition,
        ...
        if (useExponentialNotation) {
            ...
            if (repeat > 1 && repeat > minIntDigits) {
                ...
                // it is for the format 0.MMMMMxRADIX^n.
                if (exponent >= 1) {
            ...
            formatted(fieldPosition, Field.INTEGER, iFieldStart, iFieldEnd);
            if (addedDecimalSeparator) {
                formatted(fieldPosition, Field.DECIMAL_SEPARATOR, iFieldStart, iFieldEnd);
            }
            if (fFieldStart == -1) {
                fFieldStart = result.length();
            }
            formatted(fieldPosition, Field.FRACTION, iFieldStart, result.length());
            ...
            formatted(fieldPosition, Field.EXPONENT_SYMBOL, fieldStart, result.length());
            ...
            if (negativeExponent) {
                ...
                formatted(fieldPosition, Field.EXPONENT_SIGN, fieldStart, result.length());
            }
            ...
            if (sStart != result.length()) {
                formatted(fieldPosition, Field.DECIMAL_SEPARATOR, sStart, result.length());
            }
            ...
            // Record field information for caller.
            formatted(fieldPosition, Field.FRACTION, fFieldStart, result.length());
        }
    ...
    private void append(StringBuffer result, String string,
        ...
        if (string.length() > 0) {
            ...
            for (FieldPosition position : positions) {
                Format.Field attribute = position.getFieldAttribute();
                if (attribute == Field.SIGN) {
                    attribute = signAttribute;
                }
                formatted(fieldPosition, attribute, start + position.getBeginIndex(), start + position.getEndIndex());
            }
        }
    }
     ...
     */
    private boolean subparse(String text, ParsePosition parsePosition,
        ...
        if (!isExponent && text.regionMatches(position,symbols.getInfinity(),0, symbols.getInfinity().length())) {
            ...
            for (; position < text.length(); ++position) {
                ...
                int digit = Dozenal.getDigitValue(ch);
        ...
        if (!isExponent) {
            ...
            // mark success!
            parsePosition.setIndex(position + (gotPositive ? positiveSuffix.length() : negativeSuffix.length()));
        } else {
    ...
    @Override
    public Object clone() {
        DozenalFormat other = (DozenalFormat) super.clone();
    ...
    public boolean equals(Object obj) {
        ...
        DozenalFormat other = (DozenalFormat) obj;
        return ((Objects.equals(posPrefixPattern, other.posPrefixPattern) && positivePrefix.equals(other.positivePrefix))
                || (posPrefixPattern != null && posPrefixPattern.equals(other.posPrefixPattern)))
                && ((Objects.equals(posSuffixPattern, other.posSuffixPattern) && positiveSuffix.equals(other.positiveSuffix))
                || (posSuffixPattern != null && posSuffixPattern.equals(other.posSuffixPattern)))
                && ((Objects.equals(negPrefixPattern, other.negPrefixPattern) && negativePrefix.equals(other.negativePrefix))
                || (negPrefixPattern != null && negPrefixPattern.equals(other.negPrefixPattern)))
                && ((Objects.equals(negSuffixPattern, other.negSuffixPattern) && negativeSuffix.equals(other.negativeSuffix))
                    ...
                    case 2:
                        ...
                        if (inQuote) {
                            ...
                            // Next handle characters which are appended directly.
                                ...
                                multiplier = Dozenal.RADIX_SQUARED;
                                affix.append("'%");
                                continue;
                            } else if (ch == perMill) {
                                if (multiplier != 1) {
                                ...
                                multiplier = Dozenal.RADIX_CUBED;
                                affix.append("'\u2030");
                                continue;
                            } else if (ch == minus) {
                                affix.append("'-");
    ...
    private static class DigitArrays {
        static final char[] DigitOnes1000 = new char[Dozenal.RADIX_CUBED];
        static final char[] DigitTens1000 = new char[Dozenal.RADIX_CUBED];
        static final char[] DigitHundreds1000 = new char[Dozenal.RADIX_CUBED];

        // initialize on demand holder class idiom for arrays of digits
        static {
            int tenIndex = 0;
            int hundredIndex = 0;
            char digitOne = Dozenal.DIGIT_ZERO;
            char digitTen = Dozenal.DIGIT_ZERO;
            char digitHundred = Dozenal.DIGIT_ZERO;
            for (int i = 0; i < Dozenal.RADIX_CUBED; i++) {

                DigitOnes1000[i] = digitOne;
                if (digitOne == Dozenal.DIGIT_PENULTIMATE) {
                    digitOne = Dozenal.DIGIT_ZERO;
                } else {
                    digitOne++;
                }

                DigitTens1000[i] = digitTen;
                if (i == (tenIndex + Dozenal.PENULTIMATE)) {
                    tenIndex += Dozenal.RADIX;
                    if (digitTen == Dozenal.DIGIT_PENULTIMATE) {
                        digitTen = Dozenal.DIGIT_ZERO;
                    } else {
                        digitTen++;
                    }
                }
                ...
                if (i == (hundredIndex + Dozenal.RADIX_SQUARED_SUB1)) {
                    digitHundred++;
                    hundredIndex += Dozenal.RADIX_SQUARED;
                }
            }
        }
    }
    ...
    // TODO: Probably wrong for base 12
    ...
    private void formatted(FieldPosition fieldPosition, Format.Field field, int beginIndex, int endIndex) {
